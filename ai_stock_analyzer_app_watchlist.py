...
#
import os, io, math, json, time, requests, pandas as pd, numpy as np
    ...
    for url in SENATE_URL_CANDIDATES:
        try:
            ...
            for cand in ["Ticker","ticker","Symbol","symbol","AssetTicker"]:
                if cand in df.columns: ticker_col = cand; break
            ...
            if "Transaction" not in df.columns:
                for cand in ["transaction","Type","type","transaction_type"]:
                    if cand in df.columns: df["Transaction"] = df[cand]; break
                else:
                    df["Transaction"] = ""
            # Name
            if "Representative" not in df.columns:
                for cand in ["senator","Senator","Member"]:
                    if cand in df.columns: df["Representative"] = df[cand]; break
                else:
                    df["Representative"] = ""
            if "Amount" not in df.columns:
                for cand in ["amount","AmountRange","amount_range"]:
                    if cand in df.columns: df["Amount"] = df[cand]; break
                else:
                    df["Amount"] = ""
...
def gdelt_timeline_tone(query: str) -> pd.DataFrame:
    # GDELT Doc API timelinetone returns tone by day for a query (simple keyword query)
...
def tone_score(df: pd.DataFrame, days: int = 30) -> float:
    ...
    # GDELT tone is roughly centered around 0; scale to -1..1 gently
    return float(np.tanh(mean_tone / 5.0))

# -----------------------------
# Scoring
# -----------------------------
def heuristic_score(df: pd.DataFrame, horizon: str, pol_stats_combined: dict, news_component: float = 0.0) -> dict:
...
with st.sidebar:
    ...
    news_query = st.text_input("News Query (for GDELT)", value=lambda: symbol)
    use_demo = st.toggle("Load Demo Data (offline demo)", value=False)
    st.markdown("---")
    st.text_input("Alpha Vantage API Key:", value=ALPHA_KEY, key="alpha_key_input")
    if st.session_state.get("alpha_key_input"):
        os.environ["ALPHAVANTAGE_API_KEY"] = st.session_state["alpha_key_input"]
        ALPHA_KEY = st.session_state["alpha_key_input"]

# -----------------------------
# Load politician datasets once
# -----------------------------
...
def render_single_ticker(sym: str):
    ...
    # Demo or live data
    if use_demo:
        demo_csv = """date,open,high,low,close,adj_close,volume
2025-07-01,100,102,99,101,101,100000000
2025-07-02,101,103,100,102,102,110000000
2025-07-03,102,104,101,103,103,120000000
2025-07-07,103,105,102,104,104,115000000
2025-07-08,104,106,103,105,105,118000000
2025-07-09,105,107,104,106,106,119000000
2025-07-10,106,108,105,107,107,130000000
2025-07-11,107,109,106,108,108,128000000
2025-07-14,108,110,107,109,109,127000000
2025-07-15,109,111,108,110,110,126000000
2025-07-16,110,112,109,111,111,125000000
2025-07-17,111,113,110,112,112,140000000
2025-07-18,112,114,111,113,113,150000000
2025-07-21,113,115,112,114,114,145000000
2025-07-22,114,116,113,115,115,149000000
2025-07-23,115,117,114,116,116,160000000
2025-07-24,116,118,115,117,117,170000000
2025-07-25,117,119,116,118,118,165000000
2025-07-28,118,120,117,119,119,175000000
2025-07-29,119,121,118,120,120,180000000
"""
        price = pd.read_csv(io.StringIO(demo_csv), parse_dates=["date"]).set_index("date").sort_index()
    else:
        with st.spinner("Fetching price history..."):
            price = get_price_history(sym)
    if price.empty:
        st.error("No price data found. Try another ticker or set your Alpha Vantage key.")
        return
    ...
    fig_rsi = px.line(price.reset_index(), x="index", y="RSI14", labels={"index":"Date","RSI14":"RSI(14)"})
    st.plotly_chart(fig_rsi, use_container_width=True)
    fig_macd = px.bar(price.reset_index(), x="index", y="MACD_hist", labels={"index":"Date","MACD_hist":"MACD Histogram"})
    ...
    if include_news:
        ...
        with st.spinner("Fetching GDELT timelinetone..."):
            tone_df = gdelt_timeline_tone(q)
        if tone_df.empty:
            st.info("No tone data found for this query.")
        else:
            fig_tone = px.line(tone_df, x="date", y="tone", title="GDELT Tone Timeline")
...
def analyze_ticker_for_watchlist(sym: str, horizon: str) -> dict:
    ...
    if df.empty:
        return {"ticker": sym, "status":"no_data"}
    pol = combined_pol_stats(sym)
    news_c = 0.0
    if include_news:
...
def render_watchlist(horizon: str):
    ...
    # Basic sorting: strongest score first
